...
from laozhubot.plugins.setu.fetch_resources import download_database
from laozhubot.plugins.setu.permission_manager import pm
from laozhubot.plugins.setu.setu_massage import HELP_MSG
...
class ManagerHandle:
    ...
    def verify_sid(sid: str):
        ...
        try:
            ...
            # 判断是否为群聊或者私聊，并且后面的sid是否为数字
            return bool(stype in ["group", "user"] and sid.isdigit())
        except Exception:
            return False

    async def open_setu(self, matcher: Matcher, event: MessageEvent, cmd: Message = CommandArg()):
        """
        开启或关闭会话的涩图功能，白名单
        :param matcher:
        :param event:
    ...
    async def setu_db(
        ...
        try:
            ...
            remsg = f"获取 laozhu_setu.db 失败: {repr(e)}"
